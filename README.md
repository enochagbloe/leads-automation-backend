...
The public marketing waitlist uses `POST /api/waitlist` to confirm a waitlist place immediately and send a thank-you email. The legacy `POST /api/waitlist/confirm` and `POST /api/waitlist/resend` routes remain available for pending records created by earlier releases. Configure `EMAIL_FROM` with a verified sender such as `BizReply <hello@bizreplyhq.com>`.

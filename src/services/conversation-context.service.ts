...
import { ConversationScope, conversationStateService, assertConversationScope } from "./conversation-state.service";
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        return await prisma.$transaction(async tx => {
          ...
          const scopedConversation = await assertConversationScope(tx, input);
          ...
          return {
            state,
            timezone: scopedConversation.business.timezone ?? null,

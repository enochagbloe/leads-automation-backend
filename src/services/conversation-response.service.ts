...
import { conversationResponsePolicyService, fieldLabels, clarificationOptions, hasGroundedPrice, ResponseFact } from "./conversation-response-policy.service";
...
export function plannedResponseContract(plan: ConversationPlan) {
  ...
  return {
    instruction: (plan.reasonCode === "DEMO_AVAILABILITY_NOT_CONNECTED" ? "This demo cannot check live availability or book appointments. Explain that limitation without promising an external action. Avoid the word confirm or confirmation, even in a negation; the downstream booking safety gate rejects that wording. " : "") + `For this response, fulfilledPurpose MUST be ${JSON.stringify(plan.responseDirective.purpose)}, askedField MUST be ${JSON.stringify(askedField)}, questionCount MUST be ${questionCount}, and requiresHumanReview MUST be ${plan.requiresHumanReview}. ` +
      (questionCount === 0
...
export function fallbackResponse(context: AiBusinessContext, facts = responseFacts(context)): ConversationResponse | null {
  ...
  else if (p.reasonCode === "CUSTOMER_GREETING") text = "Hi! How can I help?";
  ...
  else if (p.demoSessionId && p.reasonCode === "DEMO_AVAILABILITY_NOT_CONNECTED" && p.move === "CONTINUE_WORKFLOW") text = "Live availability and appointment booking are not connected in this demo.";
...
export const conversationResponseService = {
  async generate(context: AiBusinessContext, options: Omit<AiGenerateReplyInput, "systemPrompt" | "userPrompt">) {
    ...
    const userPrompt = JSON.stringify({ clarificationOptions: clarificationOptions(plan, snapshot.state), context: JSON.parse(aiPromptContextFormatter.format(context)), responseFacts: facts, trustedWorkflowResult, tone: context.planCapabilities.tone });
